...
class Rd_Auth{
	...
	public static function createUser($user, $userInfo){
		...
		$status = (
			array_key_exists('status', $userInfo)
			? $userInfo['status']
			: ''
		);
		
		if (strpos($status,'staff') !== false) {
			$accountLevel = Account_Rd::LEVEL_STAFF;
		} else if (strpos($status,'faculty') !== false) {
			$accountLevel = Account_Rd::LEVEL_FACULTY;
		} else {
			$accountLevel = Account_Rd::LEVEL_STUDENT;
		}
		return $user->createUser($username, $firstName, $lastName, $email, $accountLevel);
	}
