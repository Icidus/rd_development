	...
	public function search($search_field, $search_term)
	{
		...
		if ($search_field == 'barcode')	{
			if ( $this->sel_item_url ) {
				...
				$url = $this->sel_item_url . "?barcode=" . urlencode($search_term);
				$curl = curl_init($url);
				curl_setopt($curl, CURLOPT_RETURNTRANSFER,true);
				curl_setopt($curl,CURLOPT_CONNECTTIMEOUT,true);
				$result = curl_exec($curl);
				...
				if  ( $status == 200 ) {
					...
					Rd_Debug::out("Setting term to $term<br />");
						...
						return new RD_Sirsi_Result(RD_Sirsi_Result::FAILURE_IDENTITY_NOT_FOUND, $result, null);
				} else {
					Rd_Debug::out("WS Status [$status] : Message: $result");
					return new RD_Sirsi_Result(RD_Sirsi_Result::FAILURE_IDENTITY_NOT_FOUND, "Status Code from SelItem Service: $status =&gt; $result", null);
				} 
			} else {
