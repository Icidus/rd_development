<?php
/*******************************************************************************
Rd/Displayer/Base.php
Base Displayer abstract class

Created by Dmitriy Panteleyev (dpantel@emory.edu) (originally baseDisplayer.php)
...
require_once(APPLICATION_PATH . '/lib/Rd/Displayer/View.php');
require_once(APPLICATION_PATH . '/lib/Rd/Displayer/Helper/Html.php');
 ...
 */
abstract class Rd_Displayer_Base {
	
	protected $_user = NULL;
	
	protected $_displayerName = 'base';
	
	/**
	 * @return void
	 * @param string $viewName name of the view script to call.
	 * @desc pass display control to a specified view script.
	 */
	public function display($viewScript,$model = array(),$fallbackToBaseViews = false)
	{
		$view = new Rd_Displayer_View($this->_displayerName, $viewScript, $this);
		...
		if ($view->isAvailable()) {
			...
			return;
		} else if ($fallbackToBaseViews){
			$view->configurePath('base', $viewScript);
			if ($view->isAvailable()){
				$this->_renderView($view);
				return;
			}		
		}
		$this->_handleError($view);
	}
	...
	protected function _handleError($view)
	{
		$errorClass = "error displayerError {$this->_displayerName}DisplayerError";
		print(
    		Rd_Debug::isEnabled()
    		? "<p class='{$errorClass}'>"
    			. "Unable to load view script: "
    			. $view->getPath() . "."
    			. "</p>"
    		: "<p class='{$errorClass}'>" //#TODO this should come from the dictionary
    			. Rd_Config::get('configurationErrorMessage')
    			. "</p>"
    			. "<p class='{$errorClass}'>"
    			. Rd_Config::get('supportMessage')
    			. "</p>"
   		);
	}
	
	public function printStatic($filename, $fallbackToBaseViews = false)
	{
		$pathInfo = pathinfo($filename);
		$basefile = $pathInfo['filename'];
		$extension = $pathInfo['extension'];
		$view = new Rd_Displayer_View($this->_displayerName, $basefile, $this);
	...
	/**
	 * @return void
	 * @param Rd_Displayer_View $view view object to render.
	 * @desc attempt to render error resulting from the view object.
	 */
	protected function _renderViewError($view){
		$e = $view->getError();
		if($e) {
			$exceptionType = get_class($e);
			print("<div class=\"viewExceptionDisplay\"><p>The View Script threw an exception: {$exceptionType}<br/>{$e->getMessage()} </p>{$view->getErrorBuffer()}<pre>{$e->getTraceAsString()}</pre></div>");
		} else {
			$extension = $view->getExtension();
			$viewType = get_class($view) . " {$view->getDisplayerName()}::{$view->getName()}" . ('php' != $extension ? "({$extension})" : '');
			print("<!-- view script {$viewType} returned no output -->");	
		}		
	}
	 ...
	 * @return void
	 * @param Rd_Displayer_View $view view object to render.
	 * @desc attempt to render the view object.
	 */
	protected function _renderView($view){
		$viewOuput = (string)$view;
		if($view->hadError() && Rd_Debug::isEnabled()){
			$this->_renderViewError($view);
		}
		print($viewOuput);
	}
	...
	/**
	 * @return void
	 * @param Rd_Displayer_View $view view object to print.
	 * @desc attempt to staticly render the view object.
	 */
	protected function _printView($view){
		$viewOuput = $view->getRaw();
		if('' == $viewOuput && Rd_Debug::isEnabled()){
			$this->_renderViewError($view); 
		} else {
			print($viewOuput);
		}
	...
	/**
	 * @return void
	 * @desc Displays the screen to inform an unrecoverable error.
	 */
	function displayError($cmd, $errorMessage, $exception=NULL) {
		$model = array(
		...
		$this->display('error', $model, true);
	}
	
	static function displayCatalogLink($reserve)
	{
		$u = $u = Account_Rd::getUserInterface();
		if ($reserve->item->getLocalControlKey()) { ?>
			[<?php self::displayQuickviewLink($reserve->item->getLocalControlKey(), 'more info'); ?>]	
		<?php } else if($u->getRole() >= Account_Rd::LEVEL_STAFF) { ?>
				[Title Control Number Missing, <a href="./?cmd=editItem&reserveID=<?php print($reserve->getId()); ?>">add it</a> to link this reserve to the catalog.]
		<?php }
	}
	
	static function displayQuickviewLink($localControlKey, $label)
	{
		$recordUrl = Rd_Registry::get('root:titleRecordUrl');
		$u = Account_Rd::getUserInterface();
		$catKey = (
			'' != trim($localControlKey) && is_numeric($localControlKey)
			? '?func=direct&doc_library=FCL01&doc_number=' . $localControlKey
			: ''
		);
		if ('' != $catKey) {
?><a id="quickViewForNCSU<?php print($localControlKey); ?>" class="external quickViewLink" data-control-key="<?php print($localControlKey); ?>" href="<?php print($recordUrl.$catKey); ?>" target="_new"><div style="display:inline;"><?php print($label); ?></div></a><?php 
		} else if($u->getRole >= Account_Rd::LEVEL_STAFF) { 
?><span class="warning">This item's control number (<?php print($localControlKey); ?>) does not match the expected format.</span><?php
		} 		
	}
	 ...
	 */
	public static function displayHiddenFields(&$hidden_fields) {				
	 ...
	 */
	public static function displayReserveRowEdit(&$reserve, $block_style='') {
		global $g_permission;
...
?>	
	<div <?php print($block_style); ?>>
		<?php if ($u->getRole() >= $g_permission['staff'] || ($reserve->getStatus() != 'DENIED' && $reserve->getStatus() != 'DENIED ALL' )) { ?>
			<div class="editOptions">
				<div class="itemNumber">
					<?php print($reserve->counter); ?>
				</div>
				<div class="checkBox">
					<input type="checkbox" name="selected_reserves[]" value="<?php print($reserve->getReserveID()); ?>" <?php print($checkbox_onchange); ?> />
				</div>
				<div class="sortBox">
					<?php print($reserve->sort_link); ?>&nbsp;
				</div>
				<div class="editBox">
					<?php print($reserve->edit_link); ?>&nbsp;
				</div>
				<div class="statusBox">
					<span class="<?php print(common_getStatusStyleTag($reserve->getStatus())); ?>"><?php print($reserve->getStatus());?></span>
				</div>
			</div>
		<?php } else { ?>
			<div class="editOptions">
...
<?php	endif; ?>
			...
			<div class="checkBox-right">
				<input type="checkbox" <?php print($checkbox_checked); ?> name="selected_reserves[]" value="<?php print($reserve->getReserveID()); ?>" <?php print($checkbox_onchange); ?> />
			</div>
...
<?php
	 ...
	 */
	public static function displayReserveRowPreview(&$reserve, $block_style='') {
		if(!($reserve->item instanceof reserveItem)) {
			$reserve->getItem();	//pull in item info
		}
...
?>
		<div class="iconBlock">
			<img src="<?php print($itemIcon); ?>" alt="icon">&nbsp;
		</div>

		<div <?php print($meta_style); ?>>
		
<?php	if($reserve->item->isPhysicalItem()){ ?>	
		
			<span class="itemTitleNoLink"><?php print($title); ?></span>
			<br />
			<span class="itemAuthor"><?php print($author); ?></span>
			<br />
			<span class="itemMeta"><?php print($callNumber); ?></span>
			<br />
			<span class="itemMetaPre">On Reserve at:</span><span class="itemMeta"><?php print($reserveDesk); ?></span>
			<?php 
			self::displayCatalogLink($reserve); 
		} else { ?>
			<?php if ($reserve->getStatus() == 'ACTIVE' || $reserve->getStatus() == 'INACTIVE'): ?>
				<a href="<?php print($viewReserveURL); ?>" target="_blank" class="itemTitle" style="margin:0px; padding:0px;"><?php print($title); ?></a>
			<?php	else: ?>
				<span class="itemTitleNoLink"><?php print($title); ?></span>
			<?php	endif; ?>
			<br />
...
<?php	if($performer) { ?>
       		...
       		<br />
       		<span class="itemMetaPre">Performed by:</span><span class="itemMeta"><?php print($performer); ?></span>
       		
<?php	} ?>
<?php	if($volTitle) { ?>

       		<br />
       		<span class="itemMetaPre">From:</span><span class="itemMeta"><?php print($volTitle); ?></span>
       		
<?php	} ?>
<?php	if($volEdition) { ?>

       		<br />
       		<span class="itemMetaPre">Volume/Edition:</span><span class="itemMeta"><?php print($volEdition); ?></span>
       		
<?php	} ?>
<?php	if($pagesTimes) { ?>

       		<br />
       		<span class="itemMetaPre">Pages/Time:</span><span class="itemMeta"><?php print($pagesTimes); ?></span>
       		
<?php	} ?>
<?php	if($source){ ?>

       		<br />
       		<span class="itemMetaPre">Source/Year:</span><span class="itemMeta"><?php print($source); ?></span>
...
<?php
	 ...
	 */
	public static function displayHeadingSelect(&$ci, $default_heading=null, $truncate_heading=false) {
...
?>
	<select name="heading_select">
		<option value=""<?php print($select_none); ?>>...</option>
		<option value="root"<?php print($select_root); ?>>Main List</option>
<?php
...
?>	
			<option value="<?php print($leaf->getID()); ?>"<?php print($select_other); ?>><?php print($label); ?></option>
<?php	endforeach; ?>
	</select>
<?php
	}
	 ...
	 */
	public static function displayDepartmentSelect($default_dept=null, $abbreviation_only=false, $field_name='department') {
		$department = new department();	//init a department object
?>
	<select name="<?php print($field_name); ?>" id="<?php print($field_name); ?>">
...
?>
		<option value="<?php print($dep->getDepartmentID()); ?>" <?php print($selected); ?>><?php print($label); ?></option>
<?php	endforeach; ?>			
	</select>
<?php
	}
	 ...
	 */
	public static function displayTermSelect($default_term=null, $show_dates=false) {
		global $calendar;
...
?>
	<!--	<option value="<?php print($term->getTermID()); ?>" <?php print($selected); ?> onclick="term_setActiveDates('<?php print($term->getBeginDate()); ?>','<?php print($term->getEndDate()); ?>')"><?php print($term->getTerm()); ?></option>	-->
		<option value="<?php print($term->getTermID()); ?>" <?php print($selected); ?>><?php print($term->getTerm()); ?></option>
<?php	endforeach; ?>			
	</select>
	
<?php	if($show_dates): //show date fields ?>

	&mdash; <input type="text" id="activation_date" name="activation_date" size="10" maxlength="10" value="<?php print($activation_date); ?>" /> <?php print($calendar->getWidgetAndTrigger('activation_date', $activation_date)); ?> &raquo; <input type="text" id="expiration_date" name="expiration_date" size="10" maxlength="10" value="<?php print($expiration_date); ?>" /> <?php print($calendar->getWidgetAndTrigger('expiration_date', $expiration_date)); ?>
	
<?php 	else:	//include them as hidden fields ?>

	<input type="hidden" id="activation_date" name="activation_date" value="<?php print($activation_date); ?>" />
	<input type="hidden" id="expiration_date" name="expiration_date" value="<?php print($expiration_date); ?>" />
...
<?php						
	 ...
	 */
	public static function displayEnrollmentSelect($default_enrollment='OPEN', $show_descriptions=false) {
		//set default
		$checked = array();
...
?>
		...
		<input type="radio" name="enrollment" id="enrollment_open" value="OPEN" <?php print($checked['OPEN']); ?> onclick="javascript: showEnrollmentOptionDescription('OPEN');" /> <span class="openEnrollment">OPEN</span>&nbsp; 
		<input type="radio" name="enrollment" id="enrollment_moderated" value="MODERATED" <?php print($checked['MODERATED']); ?> onclick="javascript: showEnrollmentOptionDescription('MODERATED');" /> <span class="moderatedEnrollment">MODERATED</span>&nbsp; 
		<input type="radio" name="enrollment" id="enrollment_closed" value="CLOSED" <?php print($checked['CLOSED']); ?> onclick="javascript: showEnrollmentOptionDescription('CLOSED');" /> <span class="closedEnrollment">CLOSED</span>&nbsp;
		<br />
		<div id="enrollment_option_desc"></div>
		
		<script language="JavaScript">
			showEnrollmentOptionDescription('<?php print($default_enrollment); ?>');
		</script>

		
<?php	else : ?>
		
		<input type="radio" name="enrollment" id="enrollment" value="OPEN" <?php print($checked['OPEN']); ?> /> <span class="openEnrollment">OPEN</span>&nbsp; 
		<input type="radio" name="enrollment" id="enrollment" value="MODERATED" <?php print($checked['MODERATED']); ?> /> <span class="moderatedEnrollment">MODERATED</span>&nbsp; 
		<input type="radio" name="enrollment" id="enrollment" value="CLOSED" <?php print($checked['CLOSED']); ?> /> <span class="closedEnrollment">CLOSED</span>&nbsp;
...
<?php	endif;
	 ...
	 */
	public static function displaySelectCourse($next_cmd, $courses=null, $msg=null, $hidden_fields=null, $ca_variable='ci') 
	{
//echo "function displaySelectCourse(next_cmd=$next_cmd, courses=$courses, msg=$msg, hf=$hidden_fields, ca_var=$ca_variable)<p> ";		
	 ...
	 */
	public static function displaySelectClass($next_cmd, $course_instances=null, $msg=null, $hidden_fields=null, $override_staff=false, $ci_variable='ci', $createClassLink=null, $selected_ci=null, $excludeCis = array()) 
	{
		global $g_permission;
		$u = Rd_Registry::get('root:userInterface');
		if(!empty($msg)) {
...
?>
		<form action="index.php" method="post" name="select_class" id="select_class">
			<input type="hidden" id="cmd" name="cmd" value="<?php print($next_cmd); ?>" />		
			<input type="hidden" id="excludeCis" <?php self::displayHiddenFields($hidden_fields); ?>>
...
<?php	
			...
			foreach($course_instances as $ci){
				if (!in_array($ci->getCourseInstanceID(), $excludeCis)) {
...
?>
						<tr class="<?php print($rowClass); ?>">
							<td style="text-align:center;"><input type="radio" id="<?php print($ci_variable); ?>" name="<?php print($ci_variable); ?>" value="<?php print($ci->getCourseInstanceID()); ?>" <?php print($selected); ?> onClick="this.form.submit.disabled=false;" /></td>
...
<?php		}
			...
			if (count($course_instances) == 0) {
?>
					<tr><td colspan="5">No courses match this criteria.<td></tr>
<?php 
			}?>
					</table>
				</td>
...
<?php 
	if (array_key_exists('asStudent', $_REQUEST)) {
?>
		<input type="hidden" name="asStudent" value="true">
<?php 
	}
?>		
		</form>	
...
<?php
	...
	public function htmlHelper($function, $parameters){ //#TODO temporary way to do this until a proper plugin method is available
		if (!is_array($parameters)){
			$parameters = array($parameters);	
		}
		return call_user_func_array("Rd_Displayer_Helper_Html::$function",$parameters);
	}
}
