...
require_once(APPLICATION_PATH . '/displayers/helpDisplayer.class.php');
require_once(APPLICATION_PATH . '/lib/Rd/Displayer/Base.php');

require_once(APPLICATION_PATH . '/lib/Service/Key.php');
require_once(APPLICATION_PATH . '/lib/Filter/DelimiterExpand.php');

class adminDisplayer extends Rd_Displayer_Base 
{

	protected $_displayerName = 'admin';
	...
	{
		$g_siteURL = Rd_Registry::get('root:mainUrl');
		$model = array(
			'rdUrl' => $g_siteURL,
			'primeServiceKey' => Service_Key::getPrimary()
		);
	...
	public function displayEditRegistrarKey($next_cmd, $courses=null, $msg=null, $hidden_fields=null, $return_variable='ca') 	
	{
		...
		print "<table class=\"table table-striped\" width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"2\" align=\"center\">\n";
...
<?php
	...
	function displayEditTerm($function, $terms, $edit_id = '')
	{	
		...
		<script language="JavaScript1.2">
			...
			function load()
			{
				<?php if (!is_null($edit_id) && '' != trim($edit_id)) { 
                    $term = new term($edit_id); 
?>
					document.getElementById('editTermArea').style.display = '';
					document.getElementById('term_id').value 	=  '<?php print($term->getTermID()); ?>';
					document.getElementById('term_name').value 	= '<?php print($term->getTermName()); ?>';
					document.getElementById('term_year').value 	= '<?php print($term->getTermYear()); ?>';
					document.getElementById('begin_date').value 		= '<?php print($term->getBeginDate()); ?>';
					document.getElementById('end_date').value 		= '<?php print($term->getEndDate()); ?>';
					document.getElementById('sort_order').value = '<?php print($term->getSortOrder()); ?>';
				...
				<td>
					<select id="term_id_select" name="term_id_select" onChange="this.form.submit();">
						...
						<?php foreach ($terms as $term) { ?>
							<option value="<?php print($term->getTermID()); ?>"> <?php print($term->getTermName()); ?> <?php print($term->getTermYear()); ?>
						<?php } ?>
					</select>
				</td>
		...
		<input type="hidden" id="term_id" name="term_id" value="">			
			...
			<table id="editTermArea" style="display: none;">
				<tr>
					...
					<td align="left">
						<select name="term_name" id="term_name">
							<?php foreach ($g_terms as $term) { ?>
								<option><?php print($term); ?></option>
							<?php } ?>
						</select>
						<input id="term_year" name="term_year" value="<?php print($last_term->getTermYear() + 1); ?>" size="4">
					</td>
				</tr>				
				<tr>
					<td>&nbsp;</td>
					<td>Begin Date:</td>
					<td>
						<input type="text" id="begin_date" name="begin_date" size="10" maxlength="10" value="<?php print($last_term->getEndDate()); ?>"/>
						<?php print($calendar->getWidgetAndTrigger('begin_date', $last_term->getEndDate())); ?>
					</td>
				...
				<tr>
					...
					<td>End Date:</td>
					<td>
						<input type="text" id="end_date" name="end_date" size="10" maxlength="10" value="<?php print($last_term->getEndDate()); ?>" />
						<?php print($calendar->getWidgetAndTrigger('end_date', $last_term->getEndDate())); ?>
					</td>
				...
				<tr>
					<td>&nbsp;</td>
					<td>Sort Order:</td>
					<td><input type="text" id="sort_order" name="sort_order" size="4" maxlength="3" value="<?php print(intval($last_term->getSortOrder()) + 1); ?>" /> </td>
					<td><i><?php print($last_term->getSortOrder()); ?> is the current max sort.  You probably want to use the next value</i></td>
				</tr>			
			</table>
	...
	function displayEditNews($function, $news, $news_item=null)
	{
		...
		<?php } else { ?>
			...
			<input type="hidden" name="news_id" id="news_id" value="<?php print($news_item['id']); ?>" />					
		<?php } ?>
		<h2>Add News</h2>
		<table>
			<tr>
				<td valign="top">Show To:</td>
					<?php if (is_null($news_item)) { ?>
						<td>
						<?php $i = 0; 
                        foreach ($g_permission as $label => $value) { if($value >= 0) { ?>						
							<label><input type="checkbox" name="permission_level[]" id="permission_level_<?php print($i); ?>" value="<?php print($value); ?>"/>
							<?php print(strtoupper($label)); 
                            ?>
                            </label>
                            <?php
							if ($i % 2){
						...
						<td valign="top"></td>
					<?php } else { //Dont allow edit of display level
							echo "<td>";
			...
			<tr>
				<td>Style:</td>
				<?php
					$styleTypes = array('notice','emergency');
					foreach($styleTypes as $styleType)
					if (!is_null($news_item))
					{
						$selected = $news_item['class'];
						$$styleType = ($selected == $styleType) ? 'selected="selected"' : '';
					} else {
						$$styleType = '';
					}
				?>
				<td>
					<select id="font_class" name="font_class">
						<option value="notice" <?php print($notice); ?>>Notice</option>
						<option value="emergency" <?php print($emergency); ?> >Emergency</option>						
					</select>				
				</td>
			</tr>
			...
			<tr>
				...
				<td>Begin:</td>
				<td>					
					<input type="text" id="begin_time" name="begin_time"  value="<?php print($begin); ?>"/>
                     (YYYY-MM-DD 24H:MIN)
					<?php print($calendar->getWidgetAndTrigger('begin_time', $begin)); ?>
					<?php
					/* allows setting begin date to null
					<input type="checkbox" id="begin_time_null" name="begin_time_null" <?php print($begin_null); ?> onclick="disable_dateSelect(this, 'begin_time');">
						Ongoing
					*/
					?>
				</td>
			...
			<tr>
				...
				<td>End:</td>
				<td>						
					<input type="text" id="end_time" name="end_time" value="<?php print($end); ?>" />
                    (YYYY-MM-DD 24H:MIN)
					<?php print($calendar->getWidgetAndTrigger('end_time', $end)); ?>
					<input type="checkbox" id="end_time_null" name="end_time_null" <?php print($end_null); ?> onclick="disable_dateSelect(this, 'end_time');">
						Ongoing
				</td>
			</tr>			
			<?php $item_sort = (is_null($news_item['sort_order'])) ? 1 : $news_item['sort_order']; ?>
			<tr>
				<td>Sort:</td>
				<td><input type="text" name="sort_order" id="sort_order" maxlength="3" size="4" value="<?php print($item_sort); ?>"/></td>
			</tr>			
			<tr>
				<td valign="top">Message Text:</td>
				<td colspan="2"><textarea name="news_text" id="news_text" wrap="virtual" cols="80" rows="6"><?php print($news_item['text']); ?></textarea></td>
			</tr>
			
			<?php $butText = (is_null($news_item)) ? "Create New" : "Submit Changes"; ?>
			<tr><td colspan="3" align="left"><input type="submit" value="<?php print($butText); ?>" /></td></tr>
		</table>
		</form>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="5" class="displayList">
			...
			<tr class="<?php print($rowClass); ?>">
				<td>
					<a href="index.php?cmd=admin&function=editNews&id=<?php print($n['id']); ?>">
						<img src="public/images/pencil.gif" alt="edit" width="24" height="20" border="0"/> Edit
				  	</a>
				</td>
				<td align="center"><?php print($p); ?></td>
				<td align="center" nowrap><?php print($b); ?></td>
				<td align="center" nowrap><?php print($e); ?></td>
				<td><?php print(htmlentities(substr($n['text'], 0, 200))); ?></td>
			</tr>
			<?php } ?>
		</table>
	...
	/**
	 * @return void
	 * @desc Displays the screen to test Cisco MXE integration.
	 */
	function displayMxeInterface($args) {
		global $g_siteURL;
		$model = (
			is_array($args) && array_key_exists('model', $args)
			? $args['model']
			: array()
		);
		$model['rdUrl'] = $g_siteURL;
		$model['messages'] = array();
		$pathToMessages = '/displayers/views/admin/messages/';
		$listOfMessages = scandir(APPLICATION_PATH . $pathToMessages);
		foreach ($listOfMessages as $filePath) {
			if ('.' != $filePath 
				&& '..' != $filePath 
				&& !is_dir(APPLICATION_PATH . $pathToMessages. $filePath)
			) {
				$message = new stdClass();
				$message->target = str_replace('_', '/', substr($filePath, 0, strpos($filePath, '.xml')));
				$message->target = Filter_DelimiterExpand::filter($message->target, '-','{','}');
				$domDoc = new DOMDocument();
				$resultDoc = $domDoc->loadXML(
					file_get_contents(APPLICATION_PATH . $pathToMessages. $filePath)//,
					//- LIBXML_NOEMPTYTAG
				);
				if ($resultDoc) {
					$xpath = new DOMXPath($domDoc);
					$rootNode = $xpath->query('/message[1]')->item(0);
					$previewNode = $xpath->query('/message/preview[1]')->item(0);
					$templateNode = $xpath->query('/message/template[1]')->item(0);
					$rootNode->removeChild($previewNode);
					$rootNode->removeChild($templateNode);
					$resultNodeSource = $domDoc->saveXML($previewNode);
					$templateNodeSource = $domDoc->saveXML($templateNode,
					- LIBXML_NOEMPTYTAG);
					$message->preview = substr(
						$resultNodeSource, 
						strlen('<preview>') , 
						strrpos($resultNodeSource,'</preview>') - strlen('<preview>')
					);
					$message->template = str_replace('{{xmlDecl}}', htmlentities('<?xml version="1.0" encoding="utf-8"?>'), trim(htmlentities(substr(
						$templateNodeSource, 
						strlen('<template>'), 
						strrpos($templateNodeSource,'</template>') - strlen('<template>')
					))));
		...
		$this->display('mxeInterface', $model);
	}
	
	/**
	 * @return void
	 * @desc Displays the javascript for testing Cisco MXE integration.
	 */
	function displayMxeScripts() {
		$this->printStatic('mxeInterface.js');
	}
	 ...
	 * @desc Displays the javascript for testing Cisco MXE integration.
	 */
	function displayhtmlScripts() {
		$this->printStatic('htmlFunctions.js');
	}
	
	function displaySwitchUser($newUser)
	{
		$model = array(
			'newUser' => $newUser
		);
		if ('' != trim(Rd_Layout::getMessage('actionResults'))) {
			$model['msg'] = Rd_Layout::getMessage('actionResults');
		}
		$this->display('switchUser', $model);
	}
	
	function displaySelectScript($scripts= array())
	{
		$model = array('scripts' => $scripts);
		if ('' != trim(Rd_Layout::getMessage('actionResults'))) {
			$model['msg'] = Rd_Layout::getMessage('actionResults');
		}
		$this->display('selectScript', $model);
	}
	
	function displayScriptResult($script)
	{
		$model = array('result' => $script->run());
		$this->display('showScript', $model);
	}
}
