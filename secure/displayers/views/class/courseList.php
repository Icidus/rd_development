...
<?php	if($this->u->getDefaultRole() >= Account_Rd::LEVEL_FACULTY){ //check DEFAULT role, so that not-trained instructors still see this tab ?>
		<li role="presentation" id="instructor_tab"><a role="tab" href="#" onclick="return showBlock('instructor_tab', 'instructor_block');">You are teaching:</a></li>
<?php	} ?>
		<li role="presentation" id="student_tab" class="current"><a role="tab" href="#" onclick="return showBlock('student_tab', 'student_block');">You are enrolled in:</a></li>
<?php	if($this->hasProxyCis) {
		 //only show proxy tab if user is currently proxying courses ?>
		<li role="presentation" id="proxy_tab"><a role="tab" href="#" onclick="return showBlock('proxy_tab', 'proxy_block');">You are proxy for:</a></li>
<?php	} ?>
	</ul>
</div>
<div class="clear"></div>
		...
		//show a radio choices for terms, to act as a filter for class list display
			...
			foreach(array_reverse(array_keys($this->instructorCis)) as $termId){
				...
				$select = ($this->instructorSelectTerm == $termId) 
					...
					: '';
	?>
			<label class="lead" style="padding-top: 10px;"><input type="radio" name="instructor_term_block" onclick="showTermBlock('instructor_block_<?php print($termId); ?>');" <?php print($select); ?> /> <?php print($term->getTermName().' '.$term->getTermYear()); ?>&nbsp;(<?php print(count($this->instructorCis[$termId])); ?> Course<?php print(count($this->instructorCis[$termId]) != 1 ? 's' : ''); ?>)</label>
	<?php		
			}
?>
				</div>
		...
		if(is_array($this->instructorCis) && count($this->instructorCis) > 0) {
			foreach($this->instructorCis as $termId=>$statusCiArray){	//split the courses by term
				$style = 
					...
					: 'style="display:none;"';
?>
		<table id="instructor_block_<?php print($termId); ?>" class="displayList table table-striped" <?php print($style); ?> width="100%">
<?php
				$rowClass = 'evenRow';
					...
					foreach($ciList as $ci){
						...
						if($this->u->getRole() < Account_Rd::LEVEL_FACULTY) {	//if the users's effective role is less than instructor (not-trained)
							...
							elseif($ci->getStatus() == 'CANCELED') {	//if the course has been cance led by the registrar
								...
								$edit_icon = '<i class="fa fa-pencil fa-2x" aria-hidden="true"></i>';	//show the edit icon
