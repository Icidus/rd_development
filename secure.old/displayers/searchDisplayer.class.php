...
class searchDisplayer extends Rd_Displayer_Base {
	/**
	* @return void
	* @desc Display staff search screen
	*/
	static function barcodeHider() {
		?>
			<script type="text/javascript">
				jQuery(document).ready(function(){
					function checkSearchField (thisSelect){
						if (jQuery(thisSelect).val()=='barcode'){
							jQuery(".barcodeHide").hide();
						}else{
							jQuery(".barcodeHide").show();
						}
					}
					
					jQuery("#searchFieldselect0").change(function(){
						checkSearchField(this);
					});
					checkSearchField(jQuery("#searchFieldselect0"));
				});
			</script>
		<?php 
	}
	...
	function searchForDocuments($cmd, $hidden_fields)
	{
		...
		for($i=0;$i<$numRows;$i++)
		{
			echo "			<tr valign='middle' align='left'". (($i>0)?" class='barcodeHide'":'').">\n";
			echo "				<td align='left' bgcolor='#CCCCCC'>\n";
			echo "					<select name='search[$i][field]' id='searchFieldselect{$i}'>\n";
			echo "						<option value='title' selected>Document Title</option>\n";
			if ($i===0){echo "						<option value='barcode'>Barcode</option>\n";}
			...
			echo "					</select>\n";
			echo "				</td>\n";
			echo "				<td align='left' bgcolor='#CCCCCC' class='barcodeHide'>\n";
			...
			echo "					</select>\n";
			echo "				</td>\n";
			echo "				<td align='left' bgcolor='#CCCCCC'><input name='search[$i][term]' type='text' id='searchTermFocus' size='40'></td>\n";
			echo "			</tr>\n";
			
			if ($i < ($numRows -1))
			{			
				echo "			<tr valign='middle' class='barcodeHide'>\n";
		...
		echo "			<tr align='left' valign='top' class='barcodeHide'>\n";
		...
		echo "				<tr class='barcodeHide'><td colspan='3' bgcolor='#CCCCCC' align='center'>&nbsp;</td></tr>\n";
	...
	function addResultsToClass($cmd, $nextCmd, $u, $selectedResults, $request, $activation_date, $expiration_date, $hidden_fields)
	{
		...
		//status and dates		
		$selected_status_inactive = (array_key_exists('status', $request) && $request['status']=='INACTIVE') ? 'checked="true"' : '';
		$selected_status_active = (array_key_exists('status', $request) && $request['status']=='INACTIVE') ? '' : 'checked="true"';
?>
		<tr>
			<td width="40%" id="statusText">
				<strong>MARK ALL:</strong>&nbsp;&nbsp;&nbsp;&nbsp;<em>Current Status:</em>
				<input type="radio" name="status" value="ACTIVE" <?php print($selected_status_active); ?> /> <span class="active">ACTIVE</span> &nbsp; <input type="radio" name="status" value="INACTIVE" <?php print($selected_status_inactive); ?> /> <span class="inactive">INACTIVE</span>
			&nbsp;&nbsp;&nbsp;
				<em>Active Dates:</em> <input type="text" id="activation_date" name="activation_date" size="10" maxlength="10" value="<?php print($activation_date); ?>" /> <?php print($calendar->getWidgetAndTrigger('activation_date', $activation_date)); ?> to <input type="text" id="expiration_date" name="expiration_date" size="10" maxlength="10" value="<?php print($expiration_date); ?>" />  <?php print($calendar->getWidgetAndTrigger('expiration_date', $activation_date));?> (YYYY-MM-DD)
			</td>
		</tr>
